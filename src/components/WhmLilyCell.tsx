...
function WhmLilyCellComponent({
  ...
  const lilyCount = normalizeCount(visual.chargeCount);
  const bloodCount = normalizeCount(visual.bloodCount);
  const title = usage
    ? `${skill.name}: 手動補正 ${normalizeCount(usage.stacks)} / ブラッドリリー ${bloodCount}`
    : `${skill.name}: リリー ${lilyCount} / ブラッドリリー ${bloodCount}。クリックで0から手動補正`;
  ...
  return (
    ...
    >
      ...
      <span className="mp-blood-lily-count" aria-hidden="true">
        B{bloodCount}
      </span>
...
export default memo(WhmLilyCellComponent, (prev, next) => {
  return (
    ...
    prev.visual.bloodCount === next.visual.bloodCount &&
