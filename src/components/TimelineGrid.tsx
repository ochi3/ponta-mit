...
export default function TimelineGrid({
  ...
  const jobHeaderWidth = useMemo(() => {
    ...
    for (const jobId of jobColspan.keys()) {
      ...
      const normalCount = jobCols.filter(c => !c.skill.maxStacks).length;
      const stackCount = jobCols.filter(c => c.skill.maxStacks).length;
      ...
      m.set(jobId, `max(${minHeaderW}, ${calcW})`);
    }
    return m;
  }, [cols, jobColspan]);
  ...
  const gridVisual: CellVisualState[][] = useMemo(() => {
    ...
    for (let ci = 0; ci < cols.length; ci++) {
      ...
      // Track effect and cooldown counts per row
      ...
      // Binary search to find first row with sec >= target
      ...
      // For each usage, mark the rows it affects
      for (const u of skillUsages) {
        ...
        // Mark effect range [u.t_sec, u.t_sec + dur)
        if (dur > 0) {
          ...
          for (let r = effectStartRow; r < effectEndRow; r++) {
            const row = rows[r];
            // Check lineIndex for same-second precision
            if (row.sec === u.t_sec && row.lineIndex < u.lineIndex) continue;
            effectCounts[r]++;
          }
        }

        // Mark cooldown range (u.t_sec, u.t_sec + cd], excluding effect period
        if (cd > 0) {
          const cdStartSec = u.t_sec + dur; // Cooldown starts after effect
            ...
            <thead className="mp-header-sticky">
              ...
              <tr>
                {cols.map((c) => {
                  ...
                  const hasStacks = typeof c.skill.maxStacks === "number" && c.skill.maxStacks > 0;
                  return (
                    <th
                      key={c.jobId + "::" + c.skill.id}
                      className={`mp-skill-header ${hasStacks ? "mp-skill-header--stack" : ""}`}
                    >
                    ...
                    {cols.map((c, ci) => {
                      ...
                      return (
                        <td
                          ...
                          className={`mp-cell mp-skill-cell ${hasStacks ? "mp-skill-cell--stack" : ""}`}
                        >
                          {hasStacks ? (
                            <StackCell
                              jobId={c.jobId}
